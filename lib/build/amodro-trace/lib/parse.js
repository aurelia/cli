...
function arrayToString(ary) {
    ...
    if (ary) {
        ary.forEach(function (item, i) {
            output += (i > 0 ? ',' : '') + '"' + lang_1.lang.jsEscape(item) + '"';
        });
    }
    output += ']';
    return output;
...
emptyScope = {}, mixin = lang_1.lang.mixin, hasProp = lang_1.lang.hasProp;
...
function parse(moduleName, fileName, fileContents, options) {
    options = options || {};
    //Set up source input
    var i, moduleCall, depString, moduleDeps = [], result = '', moduleList = [], needsDefine = true, astRoot = meriyah.parseScript(fileContents, { next: true, webcompat: true });
...
parse.findConfig = function (fileContents) {
    /*jslint evil: true */
    var jsConfig, foundConfig, stringData, foundRange, quote, quoteMatch, quoteRegExp = /(:\s|\[\s*)(['"])/, astRoot = meriyah.parseScript(fileContents, {
        loc: true,
        next: true,
        webcompat: true
    });
