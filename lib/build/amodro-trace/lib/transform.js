...
//     export default fn.apply(undefined,
...
/*global define */
'use strict';
var jsExtRegExp = /\.js$/g, baseIndentRegExp = /^([ \t]+)/, indentRegExp = /\{[\r\n]+([ \t]+)/, keyRegExp = /^[_A-Za-z]([A-Za-z\d_]*)$/, bulkIndentRegExps = {
    '\n': /\n/g,
    '\r\n': /\r\n/g
};
...
class transform {
    static toTransport(namespace, moduleName, path, contents, onFound, options) {
        ...
        var astRoot, contentLines, modLine, foundAnon, scanCount = 0, scanReset = false, defineInfos = [], applySourceUrl = function (contents) {
            if (options.useSourceUrl) {
                contents = 'eval("' + lang_1.lang.jsEscape(contents) +
        ...
        parse_1.parse.traverse(astRoot, function (node) {
            ...
            if (namespaceExists || parse_1.parse.isDefineNodeWithArgs(node)) {
                ...
                if (args.length === 1) {
                    ...
                    else if (parse_1.parse.isFnExpression(firstArg)) {
                        //define(function(){})
                    ...
                    needsId = false;
                    if (args.length === 2 &&
                        parse_1.parse.isFnExpression(args[1])) {
        ...
        defineInfos.forEach(function (info) {
            ...
            if (info.depAction === 'scan') {
                deps = parse_1.parse.getAnonDepsFromNode(info.factoryNode);
                if (deps.length) {
    ...
    static modifyConfig(fileContents, onConfig) {
        var details = parse_1.parse.findConfig(fileContents), config = details.config;
        if (config) {
    ...
    static objectToString(obj, options, totalIndent) {
        ...
        if (obj === null) {
            ...
            value = quote + lang_1.lang.jsEscape(obj) + quote;
        }
        else if (lang_1.lang.isArray(obj)) {
            lang_1.lang.each(obj, function (item, i) {
        ...
        else if (lang_1.lang.isFunction(obj) || lang_1.lang.isRegExp(obj)) {
            ...
            //An object
            lang_1.lang.eachProp(obj, function (v, prop) {
                value += (first ? '' : ',' + lineReturn) +
                    nextIndent +
                    (keyRegExp.test(prop) ? prop : quote + lang_1.lang.jsEscape(prop) + quote) +
                    ': ' +
