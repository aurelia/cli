...
exports.Bundle = class {
  ...
  copyFiles() {
    ...
    const output = this.bundler.project.platform.output;
    const root = this.bundler.project.paths.root;
    let filesToCopyList = this.config.dependencies
      .filter(x => x instanceof Object && x[copyProperty]);

    return Promise.resolve().then(() => {

      for (let filesToCopy of filesToCopyList) {
        let modulePath = path.posix.join(root, filesToCopy.path);
        for (let file in filesToCopy[copyProperty]) {
          let source = path.posix.join(modulePath, file);
          let target = path.posix.join(output, filesToCopy[copyProperty][file]);
