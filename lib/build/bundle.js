  ...
  copyFiles() {
    ...
    return Promise.resolve().then(() => {
      ...
      for (let filesToCopy of filesToCopyList) {
        let modulePath = path.join('aurelia_project', filesToCopy.path);
        for (let file in filesToCopy[copyProperty]) {
          let source = path.join(modulePath, file);
          let target = path.join(output, filesToCopy[copyProperty][file]);
