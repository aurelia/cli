...
exports.Bundle = class {
  ...
  write(platform) {
    ...
    return work.then(() => {
      ...
      let filesToCopyList = this.config.dependencies.filter(x => x.copy);

      for (let filesToCopy of filesToCopyList) {
        let modulePath = path.posix.join(root, filesToCopy.path);
        for (let file in filesToCopy.copy) {
          let source = path.posix.join(modulePath, file);
          let target = path.posix.join(platform.output, filesToCopy.copy[file]);
