    ...
    return work.then(() => {
      ...
      for (let i = 0; i < files.length; ++i) {
        ...
        function fileIsDependency(file) {
          ...
          if (!file.file || !file.file.bundle || !file.file.bundle.dependencies) {
            return false;
          }

          let match = file.file.bundle.dependencies.find(d => {
            d.name === file.file.description.name;
          });
        ...
        function acquireSourceMapForDependency(file) {
          ...
          let fileDirectory = path.posix.dirname(file.path);
          let converter = Convert.fromMapFileSource(file.contents.toString(), fileDirectory);
          sourceMap = converter
            ? converter.sourcemap
            : null;

          if (sourceMap != null) {
            // for some reason, path.posix.relative(from, to) does not work here
            sourceMap.sourceRoot = fileDirectory.substring(process.cwd().length);
          }
