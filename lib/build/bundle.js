  ...
  write(platform) {
    ...
    return work.then(() => {
      ...
      let mapSourceRoot = path.posix.relative(
        path.posix.join(process.cwd(), platform.root, platform.output),
        process.cwd()
      );
      ...
      return fs.writeFile(path.posix.join(platform.root, platform.output, bundleFileName), contents).then(() => {
        this.requiresBuild = false;

        if (mapContents) {
          return fs.writeFile(path.posix.join(platform.root, platform.output, mapFileName), mapContents)
            .catch(e => {
              console.log(`Unable to write the sourcemap to ${path.posix.join(platform.root, platform.output, mapFileName)}`);
            });
        }
      })
      .catch(e => {
  ...
  setIndexFileConfigTarget(platform, location) {
    ...
    try {
      ...
      if (configMatcher.test(indexFile)) {
        ...
        fs.writeFile((platform.legacy ? indexLocation : path.join(platform.root, indexLocation)), indexFile);
      } else {
