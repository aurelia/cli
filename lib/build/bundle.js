...
const dependency_inclusion_1 = require("./dependency-inclusion");
const Utils = __importStar(require("./utils"));
const aurelia_logging_1 = require("aurelia-logging");
const logger = (0, aurelia_logging_1.getLogger)('Bundle');
class Bundle {
    bundler;
    includes;
    excludes;
    ...
    constructor(bundler, config) {
        ...
        if (includes instanceof Array) {
            this.excludes = [];
            this.includes = includes.map(x => new source_inclusion_1.SourceInclusion(this, x));
        }
        else {
            this.excludes = (includes.exclude || []).map(x => this.createMatcher(x));
            this.includes = includes.include.map(x => new source_inclusion_1.SourceInclusion(this, x));
        }
        this.buildOptions = new configuration_1.Configuration(config.options, bundler.buildOptions.getAllOptions());
        this.requiresBuild = true;
        this.fileCache = {};
    }
    static async create(bundler, config) {
        ...
        const descriptions = await Utils.runSequentially(dependenciesToBuild, dep => bundler.configureDependency(dep));
        await Utils.runSequentially(descriptions, description => {
    ...
    async addDependency(description) {
        this.dependencies.push(description);
        const inclusion = new dependency_inclusion_1.DependencyInclusion(this, description);
    ...
    getDependencyInclusions() {
        return this.includes.filter(inclusion => inclusion instanceof dependency_inclusion_1.DependencyInclusion);
    }
    getAliases() {
        const aliases = Object.assign({}, this.aliases);
        this.includes.forEach(inclusion => {
            if (inclusion instanceof dependency_inclusion_1.DependencyInclusion && inclusion.conventionalAliases) {
    ...
    getRawBundledModuleIds() {
        const allModuleIds = new Set(this.includes.reduce((a, b) => a.concat(b.getAllModuleIds()), []));
    ...
    getBundledFiles() {
        ...
        const bundleFiles = uniqueBy(this.includes.reduce((a, b) => a.concat(b.getAllFiles()), []), file => file.path).sort((a, b) => {
    ...
    write(platform) {
        ...
        return work.then(async () => {
            const Concat = (await Promise.resolve().then(() => __importStar(require('concat-with-sourcemaps')))).default;
            ...
            for (let i = 0; i < files.length; ++i) {
                ...
                function acquireSourceMapForDependency(file) {
                    ...
                    let converter;
                    try {
                        converter = Convert.fromMapFileSource(file.contents.toString(), (filename) => fs.readFileSync(path.resolve(parsedPath.dir, filename), 'utf-8'));
                    }
                    catch (e) {
                        logger.error(e);
                        return null;
            ...
            const mapSourceRoot = path.posix.relative(path.posix.join(process.cwd(), platform.output), process.cwd());
            ...
            if (buildOptions.isApplicable('minify')) {
                ...
                const minificationOptions = { compress: false };
    ...
    async getFileFromCacheOrLoad(x) {
