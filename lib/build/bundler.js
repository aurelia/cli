...
exports.Bundler = class {
  ...
  write() {
    ...
    let targetIndex = this.project.build.targets.findIndex(x => x.id === target);
    if (targetIndex < 0) targetIndex = 0;

    return Promise.all(this.bundles.map(x => x.write(this.project.build.targets[targetIndex])));
  }
