...
exports.Bundler = class {
  ...
  write() {
    let target = this.cliOptions.getTarget();
    return Promise.all(this.bundles.map(x => x.write(target)))
      .then(() => this.writeResources(target));
  }

  writeResources(target) {
    for (let resource of target.resources) {
      this.copyAllMatchingResources(resource, target.root);
    }
    return Promise.resolve();
  }
  ...
  copyAllMatchingResources(resource, target) {
    return new Promise(resolve => {
      ...
      let root = path.resolve(this.project.platform.baseDir || '');
      let pattern = path.resolve(root, resource);

      let copy = (file, cb) => {
        let filePath = file.path;
        let targetFile = path.join(root, target, filePath.replace(root, '.'));
        ...
        if (!path.extname(targetFile) || !path.extname(targetFile).length) {
          ...
          console.log("Copying", filePath.replace(root, '.').replace(/\\/g, '/'));
