...
exports.CLI = class {
  ...
  run(commandName, commandArgs) {
    return this._establishProject(this.options).then(project => {
      if (project) {
        this.project = project;
        this.container.registerInstance(Project, project);
      }

      return this.createCommand(commandName).then(command => {
  ...
  createCommand(commandName) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (this.project) {
          this.project.locateTask(commandName).then(taskPath => {
            if (taskPath) {
              Object.assign(this.options, {taskPath:taskPath});
