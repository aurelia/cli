...
class CLI {
    ...
    async run(cmd, args) {
        const version = `${this.options.runningGlobally ? 'Global' : 'Local'} aurelia-cli v${((await Promise.resolve().then(() => __importStar(require('../package.json')))).default).version}`;
    ...
    async createCommand(commandText, commandArgs) {
        ...
        try {
            const alias = (await Promise.resolve().then(() => __importStar(require('./commands/alias.json'))))[commandModule];
            const found = this.container.get((await Promise.resolve(`${`./commands/${alias || commandModule}/command`}`).then(s => __importStar(require(s)))).default);
            ...
            if (this.project) {
                ...
                if (taskPath) {
                    ...
                    // need to configure logger after getting args
                    this.configureLogger();
                    return this.container.get((await Promise.resolve().then(() => __importStar(require('./commands/gulp')))).default);
                }
                else {
    ...
    async createHelpCommand() {
        return this.container.get((await Promise.resolve().then(() => __importStar(require('./commands/help/command')))).default);
    }
