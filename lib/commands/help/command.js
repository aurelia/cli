...
class default_1 {
    ...
    async getGlobalCommandText() {
        const commands = await Promise.all([
            Promise.resolve().then(() => __importStar(require('../new/command.json'))),
            Promise.resolve().then(() => __importStar(require('./command.json')))
        ]);
        return string.buildFromMetadata(commands.map(c => c.default), this.ui.getWidth());
    }
    async getLocalCommandText() {
        const commands = await Promise.all([
            Promise.resolve().then(() => __importStar(require('../generate/command.json'))),
            Promise.resolve().then(() => __importStar(require('../config/command.json'))),
            Promise.resolve().then(() => __importStar(require('./command.json')))
        ]);
