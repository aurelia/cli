...
Note the command might not supply all the selected features, as some of them could be from default offering.`);
  }
...
${cmd('--install-deps yarn')}, or ${cmd('--install-deps npm')}, or shorter form ${cmd('-i npm')}.`);
  ...
  async logTitle(icon, message) {
    await this.ui.log(icon + ' ' + c.bgGreen.white.bold(' ' + message + ' '));
  }
