...
exports.ProjectTemplate = class {
  constructor(model) {
    ...
    this.dist = ItemTemplate.directory('dist');
    this.scripts = ItemTemplate.directory('scripts');

    this.resources = ItemTemplate.directory('resources');
    this.elements = ItemTemplate.directory('elements');
    this.attributes = ItemTemplate.directory('attributes');
    this.valueConverters = ItemTemplate.directory('value-converters');
    this.bindingBehaviors = ItemTemplate.directory('binding-behaviors');
    this.src = ItemTemplate.directory('src').add(
      this.resources.add(
        this.elements,
        this.attributes,
    ...
    this.tests = ItemTemplate.directory('test').add(
      this.unitTests,
      this.e2eTests
    );

    this.tasks = ItemTemplate.directory('tasks');
    this.generators = ItemTemplate.directory('generators');
    thie.environments = ItemTemplate.directory('environments');

    this.projectFolder = ItemTemplate.directory('aurelia_project').add(
      this.tasks,
      this.generators,
      this.environments
      ItemTemplate.jsonObject('aurelia.json', this.model)
    );
  }
