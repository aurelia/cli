...
exports.ProjectTemplate = class {
  ...
  create(location) {
    ...
    this.model.paths = Object.assign(this.model.paths, {
      ...
      output: this.scripts.calculateRelativePath(this.projectFolder.parent),
      e2eSpecsSource: path.join(e2eRoot, 'src/**/*' + this.model.transpiler.fileExtension),
  ...
  install(ui, skip) {
    ...
    return installDependencies(ui, workingDirectory, this.clientDependencies, 'save', skip)
      .then(() => installDependencies(ui, workingDirectory, this.devDependencies, 'save-dev', skip))
      .then(() => installDependencies(ui, workingDirectory, this.dependencies, 'save', skip))
      .then(() => installDependencies(ui, workingDirectory, this.peerDependencies, 'save-peer', skip))
      .then(() => runPostInstallProcesses(ui, workingDirectory, this.postInstallProcesses));
  }
}

function installDependencies(ui, workingDirectory, dependencies, flag, skip) {
  if (skip) {
    for(let i = 0, ii = dependencies.length; i < ii; ++i) {
      ui.log(`Skipping install: ${dependencies[i]}`);
    }
