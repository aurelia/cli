...
exports.ProjectItem = class {
  ...
  buildReadmeIfExists(source, target, targetDir) {
    this._fileExistsStrategy = 'readme';
    this._readme = {
      source: source,
      target: target,
      directory: targetDir
  ...
  asTemplate(model, options) {
    ...
    this.transformers.push(content => {
      ...
      return preprocessor.preprocess(content, this.normalizeForPreprocess(model), options);
    });

    return this;
  }

  // preprocess does not support hyphens in object keys
  normalizeForPreprocess(model) {
    if (model && model.constructor === Object) {
      ...
      for (const key in keys) {
        if (key.indexOf('-') > -1) {
          ...
          model[newKey] = this.normalizeForPreProcess(model[key]);
          delete model[key];
        }
      }
    }
  ...
  _write(fullPath, content, ui) {
    ...
    return fs.stat(fullPath).then(() => {
      ...
      case 'skip':
        return Promise.resolve();
      case 'readme':
        this._readme.directory.add(this._readme.target);
        return this._readme.source.getSourceContent().then(text => {
          this._readme.target.appendText(text);
        });
      case 'merge':
