...
module.exports = class {
      ...
      .then(answer => {
        if (answer.value === 'yes') {
          ...
          return project.create(this.ui, this.options.hasFlag('here') ? undefined : process.cwd())
            ...
            .then(() => this.renderManualInstructions(project))
  ...
  renderManualInstructions(project) {
    let instructions = project.manualInstructions.getText();
    if (instructions) {
      this.ui.log(transform('<red><bgWhite>Manual changes are necessary:</bgWhite></red>' + os.EOL));
      this.ui.log(instructions + os.EOL);
      this.ui.log(transform(`These instructions can also be found in a file called ${project.manualInstructions.name} in the project directory`));
    }
  }
};
