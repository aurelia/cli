...
module.exports = class {
  ...
  displayCompletionMessage(project) {
    ...
    switch (project.model.type) {
    case 'project:plugin':
      message += this.getPluginInstructions(project);
      break;

    case 'project:application':
    default:
      message += this.getApplicationInstructions(project);
      break;
    }
  ...
  getApplicationInstructions(project) {
    let message = '';
  ...
  getPluginInstructions(project) {
    return ' Run <magenta><bold>au build</bold></magenta> to build your plugin.'
      + ' If you want to run your plugin\'s unit tests, run <magenta><bold>au test</bold></magenta>.'
      + ' That\'s just about all there is to it. If you need help, simply run <magenta><bold>au help</bold></magenta>.';
  }
};
