...
export class transform {
    static toTransport(namespace, moduleName, path, contents, onFound, options) {
       ...
       (parse as any).traverse(astRoot, function (node) {
            ...
            if (namespaceExists ||(parse as any).isDefineNodeWithArgs(node)) {
                ...
                if (args.length === 1) {
                    if (firstArg.type === 'Identifier') {
                        ...
                        needsId = true;
                        depAction = 'empty';
                    } else if ((parse as any).isFnExpression(firstArg)) {
                        //define(function(){})
                    ...
                    needsId = false;
                    if (args.length === 2 &&
                       (parse as any).isFnExpression(args[1])) {
        ...
        defineInfos.forEach(function (info) {
            ...
            if (info.depAction === 'scan') {
                deps =(parse as any).getAnonDepsFromNode(info.factoryNode);
